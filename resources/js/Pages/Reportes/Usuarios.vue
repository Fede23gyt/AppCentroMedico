<template>
    ...
    <AuthenticatedLayout>
        <div class="max-w-7xl mx-auto px-4 py-6">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Reporte de Usuarios</h1>
                <p class="text-gray-600">Análisis de actividad por usuario</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-sm p-6 text-white">
                    ...
                    <p class="text-3xl font-bold mt-2">{{ stats.total_usuarios_activos }}</p>
                </div>
                <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-sm p-6 text-white">
                    <p class="text-sm opacity-90">Con Órdenes</p>
                    <p class="text-3xl font-bold mt-2">{{ stats.usuarios_con_ordenes }}</p>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                <div class="p-6 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">Actividad por Usuario</h3>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-700">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-right text-xs font-medium text-white uppercase">Cajas</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="usuario in usuarios" :key="usuario.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ usuario.sucursal || '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">{{ usuario.total_ordenes }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">{{ usuario.total_cajas }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-green-600 text-right">${{ formatNumber(usuario.total_ventas) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
const aplicarFiltros = () => {
    router.get(route('reportes.usuarios'), filtros, { preserveState: true });
};
